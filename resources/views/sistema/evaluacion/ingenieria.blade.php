...
@section('title')
    .:: Evaluación de ingenieria de diseño ::.
@endsection
...
@section('content')
    ...
    <div class="modal fade" id="detalleEvaIngenieria" tabindex="-1" role="dialog" aria-labelledby="simpleModalLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    <div class="card card-bordered style-primary">
                        <div class="card-head">
                            ...
                            <header>Detalle Evaluación de Ingenieria de Diseño</header>
                        </div><!--end .card-head -->
                        <div class="card-body style-default-bright">
                            <div id="divEvaIngDet"></div>
                        </div><!--end .card-body -->
                    </div><!--end .card -->
                </div>
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <form class="form form-validate" role="form" id="addEmp" novalidate >
                        ...
                        <input type="hidden" name="id_evaingenieria" id="id_evaingenieria">
                        ...
                        <div class="card">
                            <div class="card-head style-primary">
                                ...
                                <header>Evaluación de Ingenieria de Diseño</header>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-sm-6">
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <select id="mdescriptiva" name="mdescriptiva" class="form-control objeto" required>
                                                <option value="">&nbsp;</option>
                                                <option value="SI">SI</option>
                                                <option value="NO">NO</option>
                                            </select>
                                            <label for="mdescriptiva">Tiene memoria descriptiva del proyecto</label>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <select id="planos" name="planos" class="form-control objeto" required>
                                                <option value="">&nbsp;</option>
                                                <option value="SI">SI</option>
                                                <option value="NO">NO</option>
                                            </select>
                                            <label for="planos">Tiene planos</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <select id="computos" name="computos" class="form-control objeto" required>
                                                <option value="">&nbsp;</option>
                                                <option value="SI">SI</option>
                                                <option value="NO">NO</option>
                                            </select>
                                            <label for="computos">Tiene cómputos</label>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <select id="otros" name="otros" class="form-control objeto" required>
                                                <option value="">&nbsp;</option>
                                                <option value="SI">SI</option>
                                                <option value="NO">NO</option>
                                            </select>
                                            <label for="otros">Tiene otros</label>
                                        </div>
                                    </div>
                                </div>
       ...
       $(function() {
            ...
            table = $('#dt_emp').DataTable({
               ...
               dom: 'Blfrtip',
                ...
                ajax: {
                    url: '{!! route('sistema.eproyecto.ingenieria.data') !!}',
                    data: function (d) {
           ...
           addEvaIngenieria= function(id, ced, nom){
               $.ajax({
                   type: "POST",
                   url:'{!! route('sistema.eproyecto.ingenieria.find') !!}',
                       ...
                       if(data.sol_id != 0){
                           if(data.msj != '0' ){
                               ...
                               $.each(data.info, function(i, item) {
                                   $("#id_evaingenieria").val(item.id);
                                   $("#id_sol").val(item.sol_id);
                                   $("#fecha").val(item.fecha);

                                   $('#mdescriptiva').append($('<option>').text(item.mdescriptiva).attr({value: item.mdescriptiva, selected: "selected"}));
                                   $('#planos').append($('<option>').text(item.planos).attr({value: item.planos, selected: "selected"}));
                                   $('#computos').append($('<option>').text(item.computos).attr({value: item.computos, selected: "selected"}));
                                   $('#otros').append($('<option>').text(item.otros).attr({value: item.otros, selected: "selected"}));
           ...
           $('#btnAddOfic').click(function(){
               if(fAddEmp.valid()){
                   $.ajax({
                       type: "POST",
                       url:'{!! route('sistema.eproyecto.ingenieria.add') !!}',
           ...
           $('#formAddEmp').on('hidden.bs.modal', function (e) {
               if($('#id_evaingenieria').val() != ''){
                   $("#mdescriptiva option:last, #planos option:last, #computos option:last, #otros option:last, #responsable option:last, #verificado option:last").remove();
                   $('#id_evaingenieria').val('');
               }
               fAddEmp.clearForm();
               $("#observacion").val('SIN OBSERVACION');
           })

           viewEvaIngenieria= function(id){
               $.ajax({
                   type: "POST",
                   url:'{!! route('sistema.eproyecto.ingenieria.view') !!}',
                   data: {"_token": "{{ csrf_token() }}", "id": id},
                   success: function( data ) {
                       if(data.sol_id != 0){
                           if(data.msj != '0' ){
                               ...
                               $.each(data.info, function(i, item) {
                                   ...
                                   html = "<div class='row'>" +
                                       ...
                                       "</div>"+
                                       "<div class='row'>" +
                                       "<div class='col-md-6'>Memoria descriptiva: <strong class='text-ultra-bold'>"+item.mdescriptiva+"</strong></div>" +
                                       "<div class='col-md-6'>Planos: <strong class='text-ultra-bold'>"+item.planos+"</strong></div>" +
                                       "</div>"+
                                       "<div class='row'>" +
                                       "<div class='col-md-6'>Computos: <strong class='text-ultra-bold'>"+item.computos+"</strong></div>" +
                                       "<div class='col-md-6'>Otros: <strong class='text-ultra-bold'>"+item.otros+"</strong></div>" +
                                       "</div>"+
                                       "<div class='row'>" +
                                   ...
                                   $('#divEvaIngDet').empty().html(html);

                               })
                               $('#detalleEvaIngenieria').modal('show');
                           }
           ...
           $('#detalleEvaIngenieria').on('hidden.bs.modal', function (e) {
               $('#divEvaEcoDet').empty();
           })

           delEvaIngenieria = function(id){
               swal({
                   ...
                   $.ajax({
                       type: "POST",
                       url:'{!! route('sistema.eproyecto.ingenieria.del') !!}',
                       data: {"_token": "{{ csrf_token() }}", "id": id},
                       success: function( data ) {
                           if(data.status == 1){
                               swal(data.msg, "", "success");
